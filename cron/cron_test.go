...
import (
	...
	nlog "github.com/meilihao/golib/v2/log"
	"go.uber.org/zap"
)

func init() {
	nlog.Level.SetLevel(zap.DebugLevel)
}
...
func (d DummyJob) ID() string                   { return "" }
func (d DummyJob) Next(now time.Time) time.Time { return time.Time{} }

func (d DummyJob) EndTime() time.Time { return time.Date(9999, 1, 1, 0, 0, 0, 0, time.Local) }
...
func (t testJob) ID() string                   { return "" }
func (t testJob) Next(now time.Time) time.Time { return time.Time{} }

func (t testJob) EndTime() time.Time { return time.Date(9999, 1, 1, 0, 0, 0, 0, time.Local) }
